...
> = {
  ...
  readonly getGroupRowContextMenuItems?: (
    row: Table.GroupRow<R>,
    node: Table.RowNode,
  ) => Table.MenuItemDef[];
  readonly getModelRowContextMenuItems?: (
    row: Table.ModelRow<R>,
    node: Table.RowNode,
  ) => Table.MenuItemDef[];
  readonly getMarkupRowContextMenuItems?: (
    row: Table.MarkupRow<R>,
    node: Table.RowNode,
  ) => Table.MenuItemDef[];
...
const useContextMenu = <R extends Table.RowData, M extends Model.RowHttpModel = Model.RowHttpModel>(
      ...
      (node: Table.RowNode): Table.ModelRow<R>[] => {
        ...
        if (tabling.rows.isModelRow(firstRow)) {
          ...
          if (!isNil(params.apis)) {
            ...
            while (
              !isNil(currentNode) &&
              !isNil(currentNode.rowIndex) &&
              currentNode.rowIndex >= 1
            ) {
        ...
        if (!isNil(onGroupRows)) {
          ...
          if (!isNil(groupRow)) {
            contextMenuItems = [
              ...contextMenuItems,
              {
                name: `Remove ${getRowLabel(row)} from ${getRowName(
                  groupRow,
                  groupRow.groupData.name,
                )} Subtotal`,
  ...
  const getModelRowMarkupContextMenuItems: (
    row: Table.ModelRow<R>,
    node: Table.RowNode,
  ...
  const getMarkupRowContextMenuItems: (
    row: Table.MarkupRow<R>,
    node: Table.RowNode,
  ) => Table.MenuItemDef[] = hooks.useDynamicCallback(
    (row: Table.MarkupRow<R>, node: Table.RowNode): Table.MenuItemDef[] => {
  ...
  const getModelRowContextMenuItems: (
    row: Table.ModelRow<R>,
    node: Table.RowNode,
  ) => Table.MenuItemDef[] = hooks.useDynamicCallback(
    (row: Table.ModelRow<R>, node: Table.RowNode): Table.MenuItemDef[] => {
      ...
      if (!isNil(params.apis) && !isNil(node.rowIndex)) {
        ...
        const previous: Table.ModelRow<R> | null = tabling.aggrid.getNodePreviousModelRow(
          params.apis.grid,
          node,
        );
        if (!isNil(previous)) {
  ...
  const getGroupRowContextMenuItems: (
    row: Table.GroupRow<R>,
    node: Table.RowNode,
  ) => Table.MenuItemDef[] = hooks.useDynamicCallback(
    (row: Table.GroupRow<R>, node: Table.RowNode): Table.MenuItemDef[] => {
      let contextMenuItems = [
        ...(!isNil(params.getGroupRowContextMenuItems)
          ? params.getGroupRowContextMenuItems(row, node)
          : []),
        {

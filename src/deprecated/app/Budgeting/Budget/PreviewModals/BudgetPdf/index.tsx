...
import { Document, View, Page, Tag, Text, NoDataPage } from "deprecated/components/pdf";
import {
  AccountsTable as GenericAccountsTable,
  SubAccountsTable as GenericSubAccountsTable,
} from "deprecated/components/tabling";
import { AccountsTable, AccountTable } from "deprecated/components/tabling/pdf";
...
const BudgetPdf = ({ budget, contacts, options }: BudgetPdfProps): JSX.Element => {
  const accountColumns = useMemo<AC[]>(() => {
    ...
    return tabling.columns.orderColumns(
      tabling.columns.normalizePdfColumnWidths(columns, (c: AC) =>
      ...
      (account: AM): C[] => {
        let columns = tabling.columns.normalizeColumns(SubAccountColumns, {
            ...
            pdfCellRenderer: (params: Table.PdfCellCallbackParams<R, M>) =>
              params.rawValue !== null ? (
                <Tag
                  ...
                  isPlural={
                    !isNil(params.row) &&
                    tabling.rows.isModelRow(params.row) &&
          ...
          estimated: {
            ...
            pdfChildFooter: (m: M) => ({ value: model.budgeting.estimatedValue(m) }),
          },
          variance: {
            pdfFooterValueGetter: model.budgeting.varianceValue(account),
            pdfChildFooter: (m: M) => ({ value: model.budgeting.varianceValue(m) }),
          },
          actual: {
            pdfFooterValueGetter: model.budgeting.actualValue(account),
            pdfChildFooter: (m: M) => ({ value: model.budgeting.actualValue(m) }),
          },
        });
  ...
  const showTopSheet = useMemo(
    () => isNil(options.tables) || includes(options.tables, "topsheet"),
    [options],
  );
    ...
    () =>
      filter(
        ...
        (account: Model.PdfAccount) =>
          (!(options.excludeZeroTotals === true) ||
            model.budgeting.estimatedValue(account) !== 0) &&
          (isNil(options.tables) || includes(options.tables, account.id)) &&
          filter(
            account.children,
            (subaccount: Model.PdfSubAccount) =>
              !(options.excludeZeroTotals === true) ||
              model.budgeting.estimatedValue(subaccount) !== 0,
          ).length !== 0,
      ),
    [budget, options],
  );

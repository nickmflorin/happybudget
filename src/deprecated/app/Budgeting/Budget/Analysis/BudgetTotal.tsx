...
const selectGroups = redux.simpleDeepEqualSelector(
  (state: Application.Store) => state.budget.analysis.groups.data,
);
const selectAccounts = redux.simpleDeepEqualSelector(
...
const getLabel = (obj: M) =>
  model.budgeting.isGroup(obj) ? obj.name : obj.description || obj.identifier || "";
  ...
  if (grouped === true) {
    groupDatums = reduce(
      ...
      (curr: Datum[], g: Model.Group, i: number) => {
        ...
        accountsWithoutGroup = filter(
          accountsWithoutGroup,
          (a: Model.Account) => !includes(g.children, a.id),
        );
    ...
    if (accountsWithoutGroup.length !== 0) {
      ...
      groupDatums = [
        ...groupDatums,
        getMetricDatum(metric, fakeGroup, accounts, groupDatums.length),
      ];
    }
    return groupDatums;
  }
  return map(accountsWithoutGroup, (a: Model.Account, i: number) =>
    getMetricDatum(metric, a, accounts, i),
  );
};

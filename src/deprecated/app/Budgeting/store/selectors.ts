...
export const selectBudgetStore = createSelector(
  [
    ...
    (s: Application.Store, ctx: Omit<BudgetContext, "budgetId">) => ctx,
  ],
  (
    s: Application.Store,
...
export const selectBudgetLoading = createSelector(
  selectBudgetStore,
  (s: Modules.PublicBudget.Store | Modules.Budget.Store | Modules.Template.Store) =>
    s.detail.loading,
);
...
export const selectIndexedAccounts = createSelector(
  [
    (s: Application.Store) => s,
    (s: Application.Store, ctx: Omit<AccountActionContext, "budgetId">) => ctx,
  ],
  (s: Application.Store, ctx: Omit<AccountActionContext, "budgetId">) =>
    selectBudgetStore(s, ctx).account,
...
export const selectRawAccount = createSelector(
  ...
  (s: Application.Store, ctx: Omit<AccountActionContext, "budgetId">) =>
    selectBudgetStore(s, ctx).account[ctx.id],
) as SelectRawAccount;
...
export const selectIndexedSubAccounts = createSelector(
  [
    (s: Application.Store) => s,
    (s: Application.Store, ctx: Omit<SubAccountActionContext, "budgetId">) => ctx,
  ],
  (s: Application.Store, ctx: Omit<SubAccountActionContext, "budgetId">) =>
    selectBudgetStore(s, ctx).subaccount,
...
export const selectRawSubAccount = createSelector(
  ...
  (s: Application.Store, ctx: Omit<AccountActionContext, "budgetId">) =>
    selectBudgetStore(s, ctx).subaccount[ctx.id],
...
export const selectSubAccountsTableStore = createSelector(
  [
    (s: Application.Store) => s,
    (s: Application.Store, ctx: Omit<SubAccountsTableActionContext, "budgetId">) => ctx,
  ],
...
export const createBudgetFooterSelector = <
  B extends Model.Budget | Model.Template,
  PUBLIC extends boolean = boolean,
>(
  ctx: Omit<BudgetActionContext<B, PUBLIC>, "budgetId">,
...
export const createAccountFooterSelector = <
  B extends Model.Budget | Model.Template,
  PUBLIC extends boolean = boolean,
>(
  ctx: Omit<AccountActionContext<B, PUBLIC>, "budgetId">,
...
export const selectFringes = createSelector(
  ...
  (s: Tables.FringeTableStore) =>
    filter(s.data, (f: Table.BodyRow<Tables.FringeRowData>) =>
      tabling.rows.isModelRow(f),
    ) as Tables.FringeRow[],
);

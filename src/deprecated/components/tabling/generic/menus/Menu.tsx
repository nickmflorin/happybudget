...
>(
  ...
  const actions = useMemo(
    () => filter(evaluated, (a: Table.MenuActionObj) => a.hidden !== true),
    [evaluated],
  );

  const leftActions = useMemo(
    () => filter(actions, (a: Table.MenuActionObj) => a.location !== "right"),
    [actions],
  );
  ...
  const hasRightMenu = useMemo(
    () =>
      (!isNil(props.saving) && isNil(props.savingChangesPortalId)) ||
    ...
    >
      ...
      <ShowHide show={hasLeftMenu}>
        <div className="table-menu-left">
          ...
          {leftActions.length !== 0 && (
            <ToolbarComponent actions={leftActions} {...props.menuActionParams} />
          )}
        </div>
      </ShowHide>
      <ShowHide show={hasRightMenu}>
        <div className="table-menu-right">
          ...
          {rightActions.length !== 0 && (
            <ToolbarComponent actions={rightActions} {...props.menuActionParams} />
          )}

...
const createUserMetricsReducer =
  (
    user: import("lib/model").User,
  ): types.BasicReducer<import("lib/model").UserMetrics, actions.UserMetricsAction> =>
  (
    state: import("lib/model").UserMetrics = user.metrics,
    action: actions.UserMetricsAction,
  ): import("lib/model").UserMetrics => {
...
const createUserReducer = (
  user: import("lib/model").User | null,
): types.BasicReducer<import("lib/model").User | null, actions.UserAction> => {
  /* We only need to be concerned with metrics when the store is configured with a user, and since
     the user cannot change from null => import("lib/model").User without reconfiguring the store
     (see note below)
     we do not need to worry about updating the metrics reducer based on the presence of a user
     after the store is configured. */
  let userMetricsReducer: types.BasicReducer<
    import("lib/model").UserMetrics,
    actions.UserMetricsAction
  > | null = null;
  if (user !== null) {
    userMetricsReducer = createUserMetricsReducer(user);
  }
  return (
    state: import("lib/model").User | null = user,
    action: actions.UserAction,
  ): import("lib/model").User | null => {
    /* The user is defined when the store is configured, and will never change from a null value to
       import("lib/model").User (only from import("lib/model").User => null in the case of a
       logout).  This is because when the
...
//     /* The store is configured with the tokenId, so the tokenId in the store should never and
//        change
...
export const createApplicationReducer = (
  ...
  combineReducers<types.ApplicationStore>({
    /* ...config.AUTH_MODULE_REDUCERS,
       ...
       contacts: reducers.createAuthenticatedModelListReducer<import("lib/model").Contact>({
         initialState: initialState.initialAuthenticatedApiModelListResponseState<
         import("lib/model").Contact>(),
         actions: {
       ...
       filteredContacts: reducers.createAuthenticatedModelListReducer<import("lib/model").Contact>({
         initialState: initialState.initialAuthenticatedApiModelListResponseState<
         import("lib/model").Contact>(),
         actions: {
       ...
       subaccountUnits: reducers.createModelListReducer<import("lib/model").SubAccountUnit>({
         actions: { response: actions.responseSubAccountUnitsAction },
         initialState: initialState.initialListResponseState<import("lib/model").SubAccountUnit>(),
       }),
       ...
       actualTypes: reducers.createModelListReducer<import("lib/model").ActualType>({
         actions: { response: actions.responseActualTypesAction },
         initialState: initialState.initialListResponseState<import("lib/model").ActualType>(),
       }), */
    loading: reducers.createSimpleBooleanReducer({
      actions: { set: actions.setApplicationLoadingAction },
    }),
    user: reducers.withActionsOnly<import("lib/model").User | null, actions.UserAction>(

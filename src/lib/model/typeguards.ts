export const isModelType = <M extends Model.GenericHttpModel>(
  m: Model.GenericHttpModel,
  t: M["type"],
): m is M => (m as M).type === t;
...
): model is Model.ModelWithDescription<M> =>
  (model as Model.ModelWithDescription<M>).description !== undefined;

export const isModelWithIdentifier = <M extends Model.Model>(
  model: M | Model.ModelWithIdentifier<M>,
): model is Model.ModelWithIdentifier<M> =>
  (model as Model.ModelWithIdentifier<M>).identifier !== undefined;

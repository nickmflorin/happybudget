...
export const isMarkup = (m: Model.GenericHttpModel): m is Model.Markup => isModelType<Model.Markup>(m, "markup");
...
export const isFringe = (m: Model.GenericHttpModel): m is Model.Fringe => isModelType<Model.Fringe>(m, "fringe");

export const isGroup = (m: Model.GenericHttpModel): m is Model.Group => isModelType<Model.Group>(m, "group");
...
export const isModelWithChildren = <M extends Model.Model>(m: M): m is M & { children: M[] } =>
  (m as M & { children: M[] }).children !== undefined && Array.isArray((m as M & { children: M[] }).children);

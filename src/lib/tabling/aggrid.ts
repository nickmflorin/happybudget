...
const _getNodesInDirection = (api: Table.GridApi, node: Table.RowNode, direction: 1 | -1): Table.RowNode[] => {
  const nodes: Table.RowNode[] = [];
  if (!isNil(node.rowIndex)) {
    let nextRowNode: Table.RowNode | undefined = api.getDisplayedRowAtIndex(node.rowIndex + direction);
...
const _getRowsInDirection = <R extends Table.RowData, RW extends Table.BodyRow<R> = Table.BodyRow<R>>(
  api: Table.GridApi,
  node: Table.RowNode,
  direction: 1 | -1,
  flt?: (r: RW) => boolean
...
export const getRowsAfterNode = <R extends Table.RowData, RW extends Table.BodyRow<R> = Table.BodyRow<R>>(
  api: Table.GridApi,
  node: Table.RowNode,
  flt?: (r: RW) => boolean
): RW[] => _getRowsInDirection<R, RW>(api, node, 1, flt);

export const getRowsBeforeNode = <R extends Table.RowData, RW extends Table.BodyRow<R> = Table.BodyRow<R>>(
  api: Table.GridApi,
  node: Table.RowNode,
  flt?: (r: RW) => boolean
): RW[] => _getRowsInDirection<R, RW>(api, node, -1, flt);

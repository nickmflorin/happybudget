...
import { createAuthenticatedSubAccountsTableReducer, createPublicSubAccountsTableReducer } from "./subaccounts";
...
>(
  config: ReducerConfig<B, M, true> & {
    readonly tableActions: Redux.TableActionCreatorMap<Model.SubAccount, SubAccountsTableActionContext<B, M, true>>;
  }
): Redux.Reducer<
  Modules.AccountOrSubAccountStore<M>,
...
> => {
  ...
  return (
    ...
    a: Redux.AnyPayloadAction<ModelActionContext<M, B, true> | SubAccountsTableActionContext<B, M, true>>
  ) => ({
    table: tableReducer(s.table, a as Redux.AnyPayloadAction<SubAccountsTableActionContext<B, M, true>>),
...
> => {
  ...
  return (
    s: Modules.AccountOrSubAccountStore<M> | undefined = config.initialState,
    a: Redux.AnyPayloadAction<ModelActionContext<M, B, false> | SubAccountsTableActionContext<B, M, false>>,
    fringesStore: Tables.FringeTableStore
  ) => ({
    table: tableReducer(s.table, a as Redux.AnyPayloadAction<SubAccountsTableActionContext<B, M, false>>, fringesStore),

 ...
 *
 * @example
 * const Permissions = enumeratedLiteralsMap(["admin", "dev", "user"] as const)
 ...
 *
 * @returns {@link EnumeratedLiteralsMap<V>}
 */
export const enumeratedLiteralsMap = <V extends readonly types.EnumeratedLiteralsString[]>(
  data: V,
): types.EnumeratedLiteralsMap<V> => ({
  ...(uniq(data) as types.UniqueArray<V>).reduce(
    (prev: types.EnumeratedLiteralsMapConstants<V>, curr: V[number]) => ({
      ...prev,
      [curr.toUpperCase().replaceAll("-", "_")]: curr,
    }),
    {} as types.EnumeratedLiteralsMapConstants<V>,
  ),
  __ALL__: uniq(data) as types.UniqueArray<V>,
  contains(this: types.EnumeratedLiteralsMap<V>, v: unknown): v is V[number] {
    return typeof v === "string" && this.__ALL__.includes(v);
  },
  validate(this: types.EnumeratedLiteralsMap<V>, v: unknown): V[number] {
    if (!this.contains(v)) {
  ...
  valuesExcluding<T extends V[number] | readonly V[number][]>(
    this: types.EnumeratedLiteralsMap<V>,
    vs: T,
  ...
  without<T extends V[number]>(
    this: types.EnumeratedLiteralsMap<V>,
    vs: T[],
  ): types.EnumeratedLiteralsMap<types.ExcludeFromReadonlyArray<V, T>> {
    const d = this.__ALL__.filter(
      (v: V[number]) => !vs.includes(v as typeof vs[number]),
    ) as types.ExcludeFromReadonlyArray<V, T>;
    return enumeratedLiteralsMap(d);
  },
  extend<T extends readonly types.EnumeratedLiteralsString[]>(
    this: types.EnumeratedLiteralsMap<V>,
    vs: T,
  ): types.EnumeratedLiteralsMap<readonly [...types.UniqueArray<V>, ...T]> {

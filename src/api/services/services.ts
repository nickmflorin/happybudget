...
export const URL = {
  v1: (...args: Http.PathParams): Http.V1Url =>
    urljoin("v1", ...map(args, (arg: string | number) => String(arg)), "/") as Http.V1Url,
};
...
export const detailDeleteService =
  (
    path: Http.PathParams | ((id: number, objId: number) => Http.PathParams),
  ): Http.DetailDeleteService =>
...
export const retrieveService =
  <R extends Model.HttpModel>(
    path: Http.PathParams | ((id: number) => Http.PathParams),
  ): Http.RetrieveService<R> =>
...
): Http.PatchService<R, Http.BulkCreatePayload<P>> =>
  patchService<Http.BulkCreatePayload<P>, R>(path);

export const bulkUpdateService = <P extends Http.PayloadObj, R>(
  path: Http.PathParams,
): Http.PatchService<R, Http.BulkUpdatePayload<P>> =>
  patchService<Http.BulkUpdatePayload<P>, R>(path);

export const bulkDeleteService = <R>(
  path: Http.PathParams,
): Http.PatchService<R, Http.BulkDeletePayload> => patchService<Http.BulkDeletePayload, R>(path);

export const detailBulkCreateService = <P extends Http.PayloadObj, R>(
  path: Http.PathParams | ((id: number) => Http.PathParams),
): Http.DetailPatchService<R, Http.BulkCreatePayload<P>> =>
  detailPatchService<Http.BulkCreatePayload<P>, R>(path);

export const detailBulkUpdateService = <P extends Http.PayloadObj, R>(
  path: Http.PathParams | ((id: number) => Http.PathParams),
): Http.DetailPatchService<R, Http.BulkUpdatePayload<P>> =>
  detailPatchService<Http.BulkUpdatePayload<P>, R>(path);

export const detailBulkDeleteService = <R>(
  path: Http.PathParams | ((id: number) => Http.PathParams),
): Http.DetailPatchService<R, Http.BulkDeletePayload> =>
  detailPatchService<Http.BulkDeletePayload, R>(path);

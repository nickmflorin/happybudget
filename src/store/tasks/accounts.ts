...
> = AccountsTableActionContext<B, PUBLIC>;
...
type AuthenticatedAccountsTableActionMap<B extends Model.Template | Model.Budget> =
...
type AuthenticatedAccountsTableTaskConfig<B extends Model.Template | Model.Budget> =
  Table.TaskConfig<
    R,
    C,
    Tables.AccountTableStore,
...
export const createAuthenticatedTableTaskSet = <B extends Model.Budget | Model.Template>(
      ...
      if (!canUseCachedResponse || redux.requestActionIsForced(action)) {
        ...
        try {
          ...
          if (models.data.length === 0) {
            ...
            const response: Http.ServiceResponse<typeof api.bulkCreateBudgetChildren> =
              yield http.request(
  ...
  const bulkCreateTask: (e: Table.RowAddEvent<R>, ctx: TC<B, false>) => SagaIterator =
  ...
  function* bulkUpdateTask(
    ...
    try {
      const response: Http.ServiceResponse<typeof api.bulkUpdateBudgetChildren> =
        yield http.request(api.bulkUpdateBudgetChildren, ctx, ctx.budgetId, requestPayload);
      yield put(
  ...
  function* bulkDeleteRows(ctx: TC<B, false>, ids: number[], markupIds?: number[]): SagaIterator {
    ...
    if (!isNil(markupIds) && markupIds.length !== 0) {
      response = yield http.request(api.bulkDeleteBudgetMarkups, ctx, ctx.budgetId, {
        ids: markupIds,
      });
    }
    if (!isNil(response)) {
  ...
  function* handleGroupAddEvent(e: Table.GroupAddEvent, ctx: TC<B, false>): SagaIterator {
    const response: Model.Group = yield http.request(
  ...
  function* handleMarkupAddEvent(e: Table.MarkupAddEvent, ctx: TC<B, false>): SagaIterator {
    ...
    yield put(
      config.actions.updateBudgetInState({ id: response.parent.id, data: response.parent }, {}),
    );

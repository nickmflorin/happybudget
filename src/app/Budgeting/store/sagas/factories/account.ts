...
export const createAccountTaskSet = <B extends Model.Budget | Model.Template>(
  ...
  actions: AccountTasksActionMap<B>,
  selectAccountId: (state: Modules.Authenticated.StoreObj) => number | null,
  selectModels: (state: Modules.Authenticated.StoreObj) => C[],
  selectAutoIndex: (state: Modules.Authenticated.StoreObj) => boolean
): AccountTaskSet => {
  function* bulkCreateTask(accountId: number, e: Table.RowAddEvent<R, C>, errorMessage: string): SagaIterator {
    ...
    */
    ...
    try {
      const response: Http.BudgetBulkResponse<B, M> = yield call(
        api.bulkUpdateAccountSubAccounts,
        accountId,
        requestPayload,
        { cancelToken: source.token }
      );
  ...
  function* bulkDeleteTask(accountId: number, e: Table.RowDeleteEvent<R, C>, errorMessage: string): SagaIterator {
    const CancelToken = axios.CancelToken;
    const source = CancelToken.source();
    ...
    if (rows.length !== 0) {
      ...
      try {
        const response: Http.BudgetBulkResponse<B, M> = yield call(api.bulkDeleteAccountSubAccounts, accountId, ids, {
          cancelToken: source.token
        });
        /*
        Note: We also have access to the updated Account from the response (as response.data)
  ...
  function* handleRowAddEvent(action: Redux.Action<Table.RowAddEvent<R, C>>): SagaIterator {
    ...
    if (!isNil(accountId) && !isNil(action.payload)) {
      ...
      yield fork(bulkCreateTask, accountId, e, "There was an error creating the sub accounts.");
    }
  }
  ...
  function* handleRowDeleteEvent(action: Redux.Action<Table.RowDeleteEvent<R, C>>): SagaIterator {
    ...
    if (!isNil(action.payload) && !isNil(accountId)) {
      ...
      const rows: R[] = Array.isArray(e.payload.rows) ? e.payload.rows : [e.payload.rows];
      if (rows.length !== 0) {
        yield fork(bulkDeleteTask, accountId, e, "There was an error deleting the sub accounts.");
      }
    }
  }
  ...
  function* handleDataChangeEvent(action: Redux.Action<Table.DataChangeEvent<R, C>>): SagaIterator {
    ...
    if (!isNil(action.payload) && !isNil(accountId)) {
      ...
      if (merged.length !== 0) {
        ...
        yield fork(bulkUpdateTask, accountId, e, requestPayload, "There was an error updating the sub accounts.");
      }
    }
  }

...
export function* deleteSubAccountTask(id: number): SagaIterator {
  ...
  // We do this to show the loading indicator next to the calculated fields of the template Footer Row,
...
export function* bulkCreateAccountSubAccountsTask(id: number, rows: Table.TemplateSubAccountRow[]): SagaIterator {
  ...
  yield put(creatingSubAccountAction(true));
  try {
    const subaccounts: Model.TemplateSubAccount[] = yield call(bulkCreateSubAccountSubAccounts, id, requestPayload);
...
export function* handleSubAccountBulkUpdateTask(
  ...
  const subaccountId = yield select((state: Redux.ApplicationStore) => state.template.subaccount.id);
  if (!isNil(subaccountId) && !isNil(action.payload)) {
    ...
    const data = yield select((state: Redux.ApplicationStore) => state.template.subaccount.subaccounts.data);
    const placeholders = yield select(
      (state: Redux.ApplicationStore) => state.template.subaccount.subaccounts.placeholders
    );
...
export function* getGroupsTask(action: Redux.Action<null>): SagaIterator {
  ...
  if (!isNil(subaccountId)) {
    ...
    try {
      const response: Http.ListResponse<Model.TemplateGroup> = yield call(getSubAccountSubAccountGroups, subaccountId, {
        no_pagination: true
      });
...
export function* getSubAccountTask(action: Redux.Action<null>): SagaIterator {
  ...
  if (!isNil(subaccountId)) {
    ...
    try {
      const response: Model.TemplateSubAccount = yield call(getSubAccount, subaccountId);

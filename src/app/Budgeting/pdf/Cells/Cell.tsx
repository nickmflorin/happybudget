...
const isCallback = <R extends Table.PdfRow<C>, M extends Model.Model, C extends Model.Model, T = any>(
  prop: Table.OptionalPdfCellCallback<R, M, C, T>
): prop is Table.PdfCellCallback<R, M, C, T> => {
  return typeof prop === "function";
};

const evaluateOptionalCallbackProp = <R extends Table.PdfRow<C>, M extends Model.Model, C extends Model.Model, T = any>(
  /* eslint-disable indent */
  prop: Table.OptionalPdfCellCallback<R, M, C, T> | undefined,
  params: Table.PdfCellCallbackParams<R, M, C>
) => {
  if (isCallback(prop)) {
    return prop(params);
  }
...
export interface CellProps<R extends Table.PdfRow<C>, M extends Model.Model, C extends Model.Model> {
  readonly column: Table.PdfColumn<R, M, C>;
  readonly row: R;
  readonly location: Table.PdfCellLocation;
  readonly style?: Table.OptionalPdfCellCallback<R, M, C, Style>;
  readonly className?: Table.OptionalPdfCellCallback<R, M, C, string>;
  readonly textStyle?: Table.OptionalPdfCellCallback<R, M, C, Style>;
  ...
  readonly valueCallback?: (params: Omit<Table.PdfCellCallbackParams<R, M, C>, "value" | "rawValue">) => any;
}

const Cell = <R extends Table.PdfRow<C>, M extends Model.Model, C extends Model.Model>(
  ...
  const cellStyle = useMemo(() => {
    let cStyle = evaluateOptionalCallbackProp<R, M, C, Style>(
    ...
    return {
      ...cStyle,
      ...evaluateOptionalCallbackProp<R, M, C, Style>(props.style, fullCallbackParams)
    };
  }, [props.column]);
  ...
  return (
    <View
      className={classNames(
        evaluateOptionalCallbackProp<R, M, C, string>(
          props.isHeader === true ? props.column.headerCellProps?.className : props.column.cellProps?.className,
          fullCallbackParams
        ),
        evaluateOptionalCallbackProp<R, M, C, string>(props.className, fullCallbackParams),
    ...
    >
      <ShowHide
        hide={
          evaluateOptionalCallbackProp<R, M, C, boolean>(props.cellContentsVisible, fullCallbackParams) === false ||
          evaluateOptionalCallbackProp<R, M, C, boolean>(props.column.cellContentsVisible, fullCallbackParams) === false
        }
      >
        {props.isHeader !== true && !isNil(props.column.cellRenderer) ? (
          ...
          <Text
            className={classNames(
              ...
              evaluateOptionalCallbackProp<R, M, C, string>(props.textClassName, fullCallbackParams)
            )}
            style={{
              ...
              ...evaluateOptionalCallbackProp<R, M, C, Style>(
                props.isHeader === true ? props.column.headerCellProps?.textStyle : props.column.cellProps?.textStyle,
                fullCallbackParams
              ),
              ...evaluateOptionalCallbackProp<R, M, C, Style>(props.textStyle, fullCallbackParams)
            }}
          >
            {value}
          </Text>

...
const selectData = redux.selectors.simpleDeepEqualSelector(
  (state: Modules.Authenticated.StoreObj) => state.budget.template.account.table.fringes.data
);
const selectTableSearch = redux.selectors.simpleShallowEqualSelector(
  (state: Modules.Authenticated.StoreObj) => state.budget.template.account.table.fringes.search
);
const selectLoading = redux.selectors.simpleShallowEqualSelector(
  (state: Modules.Authenticated.StoreObj) => state.budget.template.account.table.fringes.loading
);
const selectSaving = createSelector(
  (state: Modules.Authenticated.StoreObj) => state.budget.template.account.table.fringes.deleting,
  (state: Modules.Authenticated.StoreObj) => state.budget.template.account.table.fringes.updating,
  (state: Modules.Authenticated.StoreObj) => state.budget.template.account.table.fringes.creating,
...
const FringesModal: React.FC<FringesModalProps> = ({ template, open, onCancel }) => {
  ...
  useEffect(() => {
    ...
    if (open === true) {
      dispatch(actions.template.account.requestFringesAction(null));
    }
  }, [open]);
  ...
  return (
    <GenericFringesModal
      ...
      models={data}
      search={search}
      onSearch={(value: string) => dispatch(actions.template.account.setFringesSearchAction(value))}
      saving={saving}
      onChangeEvent={(e: Table.ChangeEvent<Tables.FringeRow, Model.Fringe>) =>
        dispatch(actions.template.account.handleFringesTableChangeEventAction(e))
      }
    />
  );
};
